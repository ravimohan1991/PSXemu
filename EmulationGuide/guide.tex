...
\code{lookup} (see \ref{code:cputick}):


\begin{lstlisting}[caption={Executing opcode.}]
void CPU::tick()
{
    ...
    /* Look up the C++ function (implementing some PS1 functionality) corresponding to opcode Execute it. */
...
where \code{set\_reg} is
\begin{lstlisting}[caption={Setting the registers to value}, label={code:registerval}]
struct MEM
{
	uint reg = 0;
	uint value = 0;
...
Note that the low 16bits are set to 0. The first instruction in the BIOS
uses LUI to put 0x13 in the high 16bits of \$8 which we have achieved in
the above C++ code.  It's important as we'll see with the next instruction.

\subsection{ORI instruction}

The logs corresponding to second instruction obtained are like so:
...
\begin{lstlisting}[caption={Store Word C++ implementation}, label={code:opsw}]
void CPU::op_sw()
{
	if (!cop0.sr.IsC)
	{
...
\begin{verbatim}
[12:31:59][info] KARMA: Inside CPU::tick
[12:31:59][info] KARMA: PC: 0xbfc0023c
[12:31:59][info] KARMA: Fetching next instruction
[12:31:59][info] KARMA: Attempting to read virtual address bfc0023c using bus
[12:31:59][info] KARMA: ------ Bus read ------
[12:31:59][info] KARMA: BIOS address: 0xbfc0023c -> Physical address: 0x1fc0023c
[12:31:59][info] KARMA: Found abs_addr (1fc0023c) in BIOSes range
[12:31:59][info] KARMA: Offset 572
[12:31:59][info] KARMA: +-------------------------------------------------------
[12:31:59][info] KARMA: | Fetched instruction 
[12:31:59][info] KARMA: | value: 1082941440, 0x408c6000
[12:31:59][info] KARMA: | opcode: 16, 0x10
[12:31:59][info] KARMA: | rs: 4, 0x4
[12:31:59][info] KARMA: | rt: 12, 0xc
[12:31:59][info] KARMA: | imm: 24576, 0x6000
[12:31:59][info] KARMA: | imms: 24576, 0x6000
[12:31:59][info] KARMA: | rd: 12, 0xc
[12:31:59][info] KARMA: | sa: 0, 0x0
[12:31:59][info] KARMA: | addr: 9199616, 0x8c6000
[12:31:59][info] KARMA: | id: 0, 0x0
[12:31:59][info] KARMA: +-------------------------------------------------------
\end{verbatim}
...
You see that the current coprocessor opcode \code{0b00100} means
...
register). Those parameters are respectively in bits [20:16] (rt) and
[15:11] (rd) of the instruction.
...
At this point the \$12 register contains \code{0x00010000}\footnote{To
  see this, look at just previos instruction log which is loading upper bits
  of \$12 with \code{0x0001}.  In assembly terms, we have
  \mbox{\code{lui \$12, 0x1}}.} so this
MTC0 instruction sets bit 16 of SR which is the ``isolate cache'' (IsC)
bit.  IsC controls whether all load and store operations are targeted to
the CPU cache and do not directly access main memory.  We further note
\begin{enumerate}
\item The CPU's cache (instruction cache) is ``isolated" from the main
  memory bus, meaning writes go only to the cache and are not immediately
  reflected in main memory.
\item This isolation allows the software, such as the PS1 kernel or BIOS,
  to manipulate the instruction cache directly for performance and
  synchronization reasons.
\item To modify code in RAM (for example, self-modifying code), the PS1
  will isolate the cache and perform writes that only affect the cache
  without main memory visibility, requiring explicit cache flushes or
  invalidations later.
\item The PS1 CPU has a 4KB instruction cache\footnote{An instruction cache
    in a CPU is designed to speed up the processing of instructions by
    storing frequently used instructions close to the CPU for quick access.
    It is much faster to access than other types of memory like main RAM.}
  but notably lacks a data
  cache, so this isolation primarily involves instruction cache operations.
\item Emulators need to emulate this bit and behavior to correctly reproduce
  how games and BIOS interact with the cache; ignoring it may cause issues
  with games relying on cache isolation mechanics.
\item The IsC is isolating the CPU instruction cache from main memory by
  routing loads and stores only within the cache during certain operations,
  a feature used by the PS1 OS for controlling cache consistency and performance
\end{enumerate}

So IsC channels all the following read and write instructions towards itself.
Thus they are prevented from accessingentering main memory, which is an optimization?
We are probably in the middle of cache initialization sequence.
...
void CPU::op_mtc0()
{
	...
	bool prev_IEC = cop0.sr.IEc;// sr = status register, IEc = Interrupt enable
	...
	uint irq_mask = cop0.sr.Sw | (cop0.sr.Intr >> 2);// Intr = Hardware Interrupt Mask, Sw = software interrupt mask
...
In line 15, we are essentially setting \$cop0\_12 (sr) register to value
\code{0x00010000}, i.e IsC bit is set to 1.  If IsC bit is set, then we
don't want to be writing to main memory and sorts.  This is precisely the
reason we have conditional check in lines 3-4 of store word code \ref{code:opsw}.
...
Followed by couple of NOPs and \code{0x240a0000}, \code{0x240b0f80},
\code{0xad400000}, 0xad400\newline020, \code{0xad400030}, \code{0xad400040},
\code{0xad400050}, \code{0xad400060}, \code{0xad400070}, we focus on next
instruction.
...
offsets), which is -9 in decimal, and multiplied by 4 (as always,
the PC must be aligned to 32bits at all times)\footnote{The immediate offset
  in branch instructions like BNE is specified in instruction units, meaning
  the offset is relative to the number of instructions, not bytes.  So to
  convert the immediate from instruction count to a byte address offset, signed
  immediate value must be multiplied by 4.}. Therefore this instruction decodes to:
...
{
	took_branch = true;
	next_pc = pc + (instr.imm_s() << 2);// Offset immediates are always shifted two places to the right since `PC` addresses have to be aligned on 32bits at all times.
}
...
The logs corresponding to next instruction are
\begin{verbatim}
[12:31:59][info] KARMA: Inside CPU::tick
[12:31:59][info] KARMA: PC: 0xbfc00274
[12:31:59][info] KARMA: Fetching next instruction
[12:31:59][info] KARMA: Attempting to read virtual address bfc00274 using bus
[12:31:59][info] KARMA: ------ Bus read ------
[12:31:59][info] KARMA: BIOS address: 0xbfc00274 -> Physical address: 0x1fc00274
[12:31:59][info] KARMA: Found abs_addr (1fc00274) in BIOSes range
[12:31:59][info] KARMA: Offset 628
[12:31:59][info] KARMA: +-------------------------------------------------------
[12:31:59][info] KARMA: | Fetched instruction 
[12:31:59][info] KARMA: | value: 558497920, 0x214a0080
[12:31:59][info] KARMA: | opcode: 8, 0x8
[12:31:59][info] KARMA: | rs: 10, 0xa
[12:31:59][info] KARMA: | rt: 10, 0xa
[12:31:59][info] KARMA: | imm: 128, 0x80
[12:31:59][info] KARMA: | imms: 128, 0x80
[12:31:59][info] KARMA: | rd: 0, 0x0
[12:31:59][info] KARMA: | sa: 2, 0x2
[12:31:59][info] KARMA: | addr: 21627008, 0x14a0080
[12:31:59][info] KARMA: | id: 0, 0x0
[12:31:59][info] KARMA: +-------------------------------------------------------
\end{verbatim}

Before we even reach the target of the branch we stumble upon
instruction \code{0x214a0080}. This one is an ADDI which
...
Seems like this is a classic trick of bitwise operations to detect overflow.

\subsection{Memory loads}

\begin{verbatim}
[12:32:01][info] KARMA: Inside CPU::tick
[12:32:01][info] KARMA: PC: 0xbfc00334
[12:32:01][info] KARMA: Fetching next instruction
[12:32:01][info] KARMA: Attempting to read virtual address bfc00334 using bus
[12:32:01][info] KARMA: ------ Bus read ------
[12:32:01][info] KARMA: BIOS address: 0xbfc00334 -> Physical address: 0x1fc00334
[12:32:01][info] KARMA: Found abs_addr (1fc00334) in BIOSes range
[12:32:01][info] KARMA: Offset 820
[12:32:01][info] KARMA: +-------------------------------------------------------
[12:32:01][info] KARMA: | Fetched instruction 
[12:32:01][info] KARMA: | value: 2366177280, 0x8d090000
[12:32:01][info] KARMA: | opcode: 35, 0x23
[12:32:01][info] KARMA: | rs: 8, 0x8
[12:32:01][info] KARMA: | rt: 9, 0x9
[12:32:01][info] KARMA: | imm: 0, 0x0
[12:32:01][info] KARMA: | imms: 0, 0x0
[12:32:01][info] KARMA: | rd: 0, 0x0
[12:32:01][info] KARMA: | sa: 0, 0x0
[12:32:01][info] KARMA: | addr: 17367040, 0x1090000
[12:32:01][info] KARMA: | id: 3, 0x3
[12:32:01][info] KARMA: +-------------------------------------------------------
\end{verbatim}
