...
We now have an primitive form of BUS and our first device (BIOS) ready
...
\item Program Counter, PC, is initially set to \code{0xbfc00000}, courtesy \code{CPU::reset()}
  \ref{code::cpureset}.
\item In order to solve our issue of having multiple mappings, at different addresses,
...
\item At the physical address \code{0x1fc000000} we read useful instruction
  \code{0x3c080013}.
\item Rest of the logging is for various registers, PS1 opcode, and all that.
\end{enumerate}

If you've been using the same BIOS file as me you should have exactly
the same value of \code{0x3c080013}. If you got an other value something
is wrong with your code. In particular if you end up with \code{0x1300083c} it
